...
export default function Registro() {
  ...
  const [form, setForm] = useState({ nombreCompleto: "", correo: "", contrasena: "" });
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_AUTH_URL}/api/auth/registrar`, form);
    ...
    <main className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h2 className="text-white text-2xl font-bold mt-4">Crear cuenta</h2>
          <p className="text-gray-400 mt-1">Empieza a aprender hoy gratis</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8">
          ...
          <form onSubmit={manejarEnvio} className="space-y-5">
            <div>
              <label className="text-gray-300 text-sm font-medium block mb-2">Nombre completo</label>
              <input
                name="nombreCompleto"
                type="text"
                required
          ...
          <p className="text-center text-gray-400 text-sm mt-6">
            ¿Ya tienes cuenta?{" "}
            <a href="/auth/iniciar-sesion" className="text-purple-400 hover:text-purple-300">
              Inicia sesión
            </a>
          </p>
        </div>
